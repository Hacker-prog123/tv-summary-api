...
def process_files():
    try:
        ...
        main_file = BytesIO(request.files['main_file'].read())
        rr_file = BytesIO(request.files['rr_file'].read())
        ...
        def classify_delivery_type(row):
            ...
            if gate == 1 and status == "Under Review (approved TV)":
                return "TV Delivered"
            if gate == 1 and tvr_gate == "0":
                return "Undelivered"
            if gate == 1 and tvr_gate == "1" and status == "Under Review (approved TV)":
                return "TV Delivered"
        ...
        current_week_summary = pd.DataFrame({
            ...
            "Value": [
                ...
                round(
                    df_current_week["Delivery Type"].isin(["TV Delivered", "HU/Draft Delivered"]).sum()
                    / len(df_current_week) * 100, 2
        ...
        current_week_detail["Total Delivered"] = (
            current_week_detail["HU_Draft_Delivered"] + current_week_detail["TV_Delivered"]
        )
        current_week_detail["Total Count"] = (
            current_week_detail["Total Delivered"] + current_week_detail["Undelivered"]
        )
        current_week_detail["Delivery %"] = round(
            current_week_detail["Total Delivered"] / current_week_detail["Total Count"] * 100, 2
        )
        ...
        summary_all_df = pd.DataFrame({
            "Metric": [
                "Total TVs in List", "Total Delivered (TV or HU)",
                "TV Delivered", "HU/Draft Delivered",
